...
def get_sub_video(args, num_process, process_idx):
    ...
    os.makedirs(osp.join(args.output,
    f'{args.video_name}_inp_tmp_videos'), exist_ok=True)
...
class Reader:
    def __init__(self, args, total_workers=1, worker_idx=0):
        ...
        if self.input_type.startswith('video'):
            ...
            if self.input_type.startswith('image'):
                ...
                all_files = sorted(glob.glob(os.path.join(args.input, '*')))
                ...
                for file_path in all_files:
                    ...
                    if file_type == 'image':
                        ...
                        # For now, let's add videos to paths as well
                        self.paths.append(file_path)

                # Process paths based on the detected file types
                if self.paths:
                    if detect_file_type(self.paths[0]) == 'image':
                        # Handle image sequence as before
                        self.nb_frames = len(self.paths)
                        assert self.nb_frames > 0, 'empty folder'
                        from PIL import Image
                        tmp_img = Image.open(self.paths[0])
                        self.width, self.height = tmp_img.size
                    elif detect_file_type(self.paths[0]) == 'video':
                        # Handle video sequence or single video
                    ...
                    raise ValueError('No valid image or video files found in the input folder')

                self.idx = 0

    def initialize_from_video(self, video_path):
        self.stream_reader = (
            ffmpeg.input(video_path).output('pipe:', format='rawvideo', pix_fmt='bgr24',
                                            loglevel='error').run_async(
                                                pipe_stdin=True, pipe_stdout=True, cmd=self.args.ffmpeg_bin))
    ...
    def get_frame(self):
        ...
        elif self.input_type.startswith('folder'):
            return self.get_frame_from_stream()
        else:
...
class Writer:
    ...
    def __init__(self, args, audio, height, width, video_save_path, fps):
        try:
            ...
            print("Error initializing writer:", e)
...
def inference_video(args, video_save_path, device=None, total_workers=1, worker_idx=0):
    ...
    print("Args in inference video:", args)
    ...
    fps = reader.get_fps()
    print("Number of frames: ",len(reader))
    print("Number of fps: ",fps)
    writer = Writer(args, audio, height, width, video_save_path, fps)

    pbar = tqdm(total=len(reader), unit='frame', desc='inference',position=worker_idx)
    ...
    while True:
        ...
        try:
            ...
            print("We are here iii")
            writer.write_frame(output)

        processed_frames += 1
        progress = processed_frames / len(reader)  # Calculate progress
        # You can return the progress here or update a global variable
        # For now, I'll just print the progress
        print(f'Progress (Worker {worker_idx}): {progress:.2%}')
        if device.type == 'cuda':  # If using GPU, synchronize CUDA operations
...
def run(args):
    ...
    num_process = num_cpus * args.num_process_per_gpu
    ...
    with ProcessPoolExecutor() as executor:
        for i in range(num_process):
            ...
            future = executor.submit(inference_video, args, sub_video_save_path, torch.device('cpu'), num_process, i)
